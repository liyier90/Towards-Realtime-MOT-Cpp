...
void KalmanFilter::predict(KAL_MEAN &mean, KAL_COVA &covariance)
{
  ...
  KAL_COVA covariance1 = this->_motion_mat * covariance *
      _motion_mat.transpose();
  ...
  , const DETECTBOX &measurement)
{
  ...
  //     check_finite=False)
  // kalmain_gain = scipy.linalg.cho_solve((cho_factor, lower),
  //     np.dot(covariance, self._upadte_mat.T).T, check_finite=False).T
  Eigen::Matrix<float, 4, 8> B = (covariance * (_update_mat.transpose()))
      .transpose();
  ...
  KAL_COVA new_covariance = covariance - kalman_gain * projected_cov *
